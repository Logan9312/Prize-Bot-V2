...
func GuildCreateHandler(s *discordgo.Session, g *discordgo.GuildCreate) {
	...
	if time.Since(g.JoinedAt) < 5*time.Minute {
		...
		_, err := SuccessMessage(s, channelID, &discordgo.InteractionResponseData{
...
func MessageHandler(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	case "bid":
		...
		if len(args) != 2 {
			message, err = ErrorMessage(s, m.ChannelID, fmt.Sprintf("Invalid number of arguments passed. Need 2, used %d", len(args)))
			if err != nil {
				fmt.Println(err)
				return
			}
		...
		if err != nil {
			ErrorMessage(s, m.ChannelID, err.Error())
			fmt.Println(err)
			return
		}
		...
		if err != nil {
			fmt.Println(err)
			_, err = ErrorMessage(s, m.ChannelID, err.Error())
			if err != nil {
				fmt.Println(err)
				return
			}
		...
		if err != nil {
			fmt.Println(err)
			message, err = ErrorMessage(s, m.ChannelID, err.Error())
			if err != nil {
				fmt.Println(err)
			}
		} else {
			message, err = SuccessMessage(s, m.ChannelID, &discordgo.InteractionResponseData{
